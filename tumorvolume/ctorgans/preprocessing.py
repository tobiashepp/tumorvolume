...
h5_path = work_dir/'interim/ctorgans/ctorgans.h5'
Path(h5_path.parent/'keys').mkdir(exist_ok=True)
...
for k, (train_index, test_index) in enumerate(kf.split(subject_keys)):
    ...
    with Path(h5_path.parent/'keys'/(h5_path.stem+f'_train{k}.dat')).open('w') as keyfile:
        keyfile.writelines(train_keys)
    test_keys = [(subject_keys[i] + '\n') for i in test_index]
    with Path(h5_path.parent/'keys'/(h5_path.stem+f'_test{k}.dat')).open('w') as keyfile:
        keyfile.writelines(test_keys)

# CAVE several orientations are corrupted! -> notebooks/0.0-th-check-ct-organs.ipynb to fix this
